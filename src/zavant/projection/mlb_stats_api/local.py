"""Local Stats API revision discovery and Parquet publication."""
...
def run_local_projection(
    ...
    """Build a local-only Parquet mirror of the production projection contract.

    This operator/development path validates the same source revisions and
    table contracts as Glue, but publishes a self-contained directory rather
    than mutating Iceberg history or current-state views.
    """
    ...
    try:
        ...
        # The output becomes visible only after every table and the completion
