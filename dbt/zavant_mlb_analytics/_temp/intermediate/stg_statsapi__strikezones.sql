/*
create a table that defines landmarks of the strikezone for each batter.
used in pitching analyses that require zone-based metrics. this table can
be joined with the pitches and using together with the coord_px/coord_pz
(horizontal/vertical coordinates of the plate crossing of the pitch) to
determine which zone the pitch belongs to. this version uses a 3x3 grid inside
the zone (configurable), as well as four perimeter zones that are the width of
a baseball outside the strike zone ("one ball off the plate").
*/
...
trans1 as (
    select
        a.*,
        a.statcast_inch * 8.5 as half_plate, -- plate is 17 inches wide
        a.statcast_inch * 2.9 as ball -- baseball is 2.9 inches in diameter
...
trans3 as (
    select
        a.*,
        a.strikezone_width / (select a.nrows from dims as a) as width_each,
        a.strikezone_height / (select a.ncols from dims as a) as height_each,
...
perimeter_zone as (
    ...
    select
        a.*,
        (select a.nzones + 1 as b from dims as a) as zone_idx,
    ...
    select
        a.*,
        (select a.nzones + 2 as b from dims as a) as zone_idx,
        'perimeter' as zone_type,
    ...
    select
        a.*,
        (select a.nzones + 3 as b from dims as a) as zone_idx,
        'perimeter' as zone_type,
        a.strikezone_left as zone_left,
    ...
    select
        a.*,
        (select a.nzones + 4 as b from dims as a) as zone_idx,
        'perimeter' as zone_type,
...
outside as (
    ...
    select
        a.*,
        (select a.nzones + 5 as b from dims as a) as zone_idx,
    ...
    select
        a.*,
        (select a.nzones + 6 as b from dims as a) as zone_idx,
        'outside' as zone_type,
    ...
    select
        a.*,
        (select a.nzones + 7 as b from dims as a) as zone_idx,
        'outside' as zone_type,
        a.perimeter_left as zone_left,
    ...
    select
        a.*,
        (select a.nzones + 8 as b from dims as a) as zone_idx,
        'outside' as zone_type,
        -999 as zone_left,
        999 as zone_top,
