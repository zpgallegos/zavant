...
from zavant.ingestion.mlb_stats_api.storage.models import CurrentRawGameRevision, LandedRawGame


class PathRawGameStore:
    """Persist immutable source revisions using a deterministic path layout.

    This is the core raw-evidence boundary for Stats API ingestion. Immutable
    revision directories preserve history, while one mutable ``current.json``
    pointer identifies the revision downstream current-state views should use.
    ...
    def land(
        ...
        # Revision identity follows JSON meaning rather than transport
        # whitespace; the separate raw checksum still preserves byte evidence.
        ...
        if created or previous_revision_id is None:
            # Replaying an already-known historical revision must not roll a
            # newer current pointer backward. A newly completed revision does
            # become current, as does the bootstrap case with no pointer.
            current_pointer = {
                "canonical_sha256": canonical_checksum,
