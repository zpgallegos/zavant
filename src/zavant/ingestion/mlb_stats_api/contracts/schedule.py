    ...
    def from_bytes(cls, raw: bytes) -> "ScheduleResponse":
        ...
        for date_index, date_entry in enumerate(dates):
            ...
            for game_index, game_entry in enumerate(games):
                ...
                existing = games_by_pk.get(scheduled_game.game_pk)
                if (
                    existing is None
                    or scheduled_game.scheduled_start >= existing.scheduled_start
                ):
                    games_by_pk[scheduled_game.game_pk] = scheduled_game

        if game_entry_count != total_games:
            raise ScheduleContractError(
                "totalGames does not match the number of schedule entries"
            )
