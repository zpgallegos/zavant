...
from zavant.ingestion.mlb_stats_api.acquisition.corrected_games import (
    CorrectedGameProcessingResult,
    CorrectedGameProcessor,
)
from zavant.ingestion.mlb_stats_api.acquisition.deferred_games import (
...
from zavant.ingestion.mlb_stats_api.storage.protocols import DailyRunStore
...
class DailyAcquisitionCoordinator:
    ...
    This is the orchestration root for the scheduled Stats API workflow. It
    records branch isolation explicitly so durable correction or deferred work
    can still make progress when fresh discovery is temporarily unavailable.
    ...
    def run(
        ...
        # Branch order is deliberate: discover new corrections first, then
        # drain all durable correction/deferred work before adding schedule
        # discoveries. Each branch records its own failure and does not
        # short-circuit the remainder of the run.
