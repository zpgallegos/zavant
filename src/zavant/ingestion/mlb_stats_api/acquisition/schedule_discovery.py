...
from zavant.ingestion.mlb_stats_api.acquisition.bounded_games import (
    BoundedGameAcquirer,
    BoundedGameAcquisitionResult,
)
from zavant.ingestion.mlb_stats_api.storage.protocols import ScheduleWatermarkStore
...
class ScheduleDiscoverer:
    ...
    This class owns incremental date policy only; ``BoundedGameAcquirer`` owns
    schedule evidence and per-game work. The watermark means every date through
    it completed discovery, while deferred games remain durable separate work.
    ...
    def discover(
        ...
        if current is None:
            ...
            # The overlap catches recent schedule changes. It is not the
            # recovery mechanism for unfinished games; the deferred store is.
