...
if __name__ == "__main__":
    ...
    read = lambda dname: spark.read.format("parquet").load(f"{DATA_DIR}/{dname}")
    ...
    teams = (
        read("f_game_teams").select("team_id", "abbreviation", "league_name").distinct()
    )
    ...
    std.write.format("parquet").mode("overwrite").save(
        f"{DATA_DIR}/../zavant-products/standard_batting"
    )
