...
if __name__ == "__main__":
    ...
    # percentile qualifier: at least 2.1 plate appearances per team game
    # std["perc_qual"] = (~std.is_partial) & ((std.PA / std.team_games) >= 2.1)
    #std["perc_qual"] = (~std.is_partial) & ((std.PA / std.team_games) >= 2.1)
    std["perc_qual"] = (~std.is_partial) & (std.PA >= 100)
    ...
    perc_cols = {f"{col}_perc": float for col in stat_cols if col not in grp}
    ...
    # done, write out
    out = std[list(stat_cols.keys()) + list(perc_cols.keys())].sort_values(
        grp, ascending=[True, False, True, True]
    )
